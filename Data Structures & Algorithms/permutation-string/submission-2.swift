class Solution {
    func checkInclusion(_ s1: String, _ s2: String) -> Bool {
        ...
        var matches = 0  // distinct s1 chars whose window count == need count

        for r in 0..<s2Chars.count {
            // Expand: add s2Chars[r]
            let add = s2Chars[r]
            if let target = need[add] {
                window[add, default: 0] += 1
                let count = window[add, default: 0]
                if count == target {             // just reached target
                    matches += 1
                } else if count == target + 1 {  // just overshot it
                    matches -= 1
                }
            }

            // Slide: drop the char that fell out of the window
            if r >= windowSize {
                let drop = s2Chars[r - windowSize]
                if let target = need[drop] {
                    window[drop, default: 0] -= 1
                    let count = window[drop, default: 0]
                    if count == target {             // came back down to target
                        matches += 1
                    } else if count == target - 1 {  // just dropped below it
                        matches -= 1
                    }
                }
            }
